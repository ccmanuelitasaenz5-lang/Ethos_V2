    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            if (result.error) {
                ...
                setMessage('Cuenta creada con exito')
                form.reset()
                onSuccess?.()
            }
        } catch (err) {
            console.error('Error inesperado:', err)
            setMessage('Error inesperado. Intentalo de nuevo.')
        } finally {
            setLoading(false)
        }
    }
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label className="block text-xs font-medium text-gray-700">Codigo</label>
                ...
                <div>
                    ...
                    <select name="is_movement" required className="mt-1 block w-full text-sm border-gray-300 rounded-md shadow-sm h-9 px-2">
                        <option value="true">Si (Recibe asientos)</option>
                        <option value="false">No (Es de nivel superior)</option>
                    </select>
                </div>
            </div>
            <button type="submit" disabled={loading} className="w-full inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700 disabled:opacity-50">
